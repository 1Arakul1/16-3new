...
def remove_dog_from_profile(request, dog_id):
    ...
    dog.owner = None  # Удаляем связь с пользователем
    dog.save()  # Сохраняем изменения
    return JsonResponse({'message': 'Собака успешно удалена из профиля.'}, status=200)

@require_http_methods(["POST"])
@login_required
def add_dog_to_profile(request, dog_id):
    dog = get_object_or_404(Dog, id=dog_id)
    if dog.owner is None:  # Проверяем, что собака не принадлежит никому
        dog.owner = request.user  # Устанавливаем владельца
        dog.save()
        return JsonResponse({'message': 'Собака успешно добавлена в профиль.'}, status=200)
    return JsonResponse({'error': 'Собака уже принадлежит другому пользователю.'}, status=400)
...
def dog_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            dog.save()
            return redirect('users:user_profile')  # Изменяем редирект на страницу профиля
...
def dog_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect(reverse('users:user_profile'))  # Перенаправляем на профиль пользователя
